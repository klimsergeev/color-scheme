...
// ============================================
// Quantile Price Color Mapper
// ============================================

class QuantilePriceColorMapper extends PriceColorMapper {
  valueToHSL(value) {
    const hue = this.interpolateHue(value);
    const saturationBase = 40;
    const saturationBoost = 20 * Math.pow(Math.abs(value - 0.5) * 2, 0.7);
    const saturation = saturationBase + saturationBoost;
    const lightness = 57 - 5 * Math.pow(Math.abs(value - 0.5) * 2, 0.5);
    return { h: hue, s: saturation, l: lightness };
  }

  mapPricesToColorsQuantile(prices) {
    if (prices.length === 0) return [];
    if (prices.length === 1) {
      return super.mapPricesToColors(prices);
    }

    const sorted = [...prices].map((p, i) => ({ price: p, index: i }))
      .sort((a, b) => a.price - b.price);

    const results = new Array(prices.length);

    sorted.forEach((item, rank) => {
      const quantileRank = rank / (sorted.length - 1);
      const hsl = this.valueToHSL(quantileRank);
      const rgb = this.hslToRgb(hsl);
      ...
      results[item.index] = {
        ...
        colorRGB: rgb,
        colorHSL: hsl,
        normalizedValue: quantileRank,
        percentile: quantileRank * 100,
      };
    });
...
class HallColorMapper {
   ...
   * @param {Array} priceColors - массив объектов {price, color} от QuantilePriceColorMapper
   */
...
let currentPrices = [];
let mapper = new QuantilePriceColorMapper();
let hallMapper = null;
...
function renderPrices(containerId, prices) {
  ...
  const results = mapper.mapPricesToColorsQuantile(prices);
...
function updateHallPreview(prices) {
  ...
  const priceColors = mapper.mapPricesToColorsQuantile(prices);
